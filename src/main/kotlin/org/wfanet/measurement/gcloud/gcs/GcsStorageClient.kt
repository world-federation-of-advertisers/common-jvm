...
class GcsStorageClient(
  ...
  override suspend fun getBlob(blobKey: String): StorageClient.Blob? {
    val blob: Blob? =
      withContext(coroutineContext + CoroutineName("getBlob")) { storage.get(bucketName, blobKey) }
  ...
  private inner class ClientBlob(private val blob: Blob) : StorageClient.Blob {
    ...
    override fun read(): Flow<ByteString> {
      return blob.reader().asFlow(READ_BUFFER_SIZE, coroutineContext + CoroutineName("readBlob"))
    }

    override suspend fun delete() {
      check(withContext(coroutineContext + CoroutineName("deleteBlob")) { blob.delete() }) {
