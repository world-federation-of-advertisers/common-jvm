...
/**
 * A Google Pub/Sub client that uses the Pull API to retrieve messages from a subscription.
 *
 * This implementation exposes acknowledgment IDs, allowing manual control over ack deadline
 * extensions via [GooglePubSubClient.modifyAckDeadline].
 *
 * @param projectId The Google Cloud project ID.
 * @param googlePubSubClient The client interface for interacting with the Google Pub/Sub service.
 * @param maxMessages The maximum number of messages to pull in each request. Defaults to 10.
 * @param pullIntervalMillis The interval between pull requests in milliseconds. Defaults to 100ms.
...
class PullSubscriber(
      ...
      try {
        ...
        try {
          ...
          for (receivedMessage in pullResponse.receivedMessagesList) {
            ...
            // Send to channel
            logger.info("Sending message with ackId=$ackId to channel for subscription: $subscriptionId")
            send(queueMessage)
   ...
   *
   * @param subscriptionName The full subscription name (projects/{project}/subscriptions/{sub}).
  ...
  private class PullMessageConsumer(
    ...
    override fun ack() {
      try {
        ...
        logger.info("Failed to acknowledge message $ackId for subscription $subscriptionName: ${e.message}")
        throw e
      }
    }
    ...
    override fun nack() {
      try {
        logger.info("Nacking message with ackId=$ackId for subscription: $subscriptionName")
        ...
        logger.info("Successfully nacked message with ackId=$ackId")
      } catch (e: Exception) {
        logger.info("Failed to nack message $ackId for subscription $subscriptionName: ${e.message}")
        throw e
      }
    }
  }
