...
abstract class GooglePubSubClient : AutoCloseable {
  ...
  suspend fun deleteTopic(projectId: String, topicId: String) {
    logger.info("Deleting topic: $topicId in project: $projectId")
    val topicName = TopicName.of(projectId, topicId)
    ...
    logger.info("Successfully deleted topic: $topicId")
  }
  ...
  suspend fun modifyAckDeadline(
    ...
    logger.info("Modifying ack deadline for ${ackIds.size} message(s) in subscription: $subscriptionId to $ackDeadlineSeconds seconds")
    ...
    logger.info("Successfully modified ack deadline for ${ackIds.size} message(s)")
  }

  override fun close() {
    logger.info("Closing GooglePubSubClient")
    if (topicAdminClient.isInitialized()) {
      logger.info("Closing TopicAdminClient")
      topicAdminClient.value.close()
    }
    if (subscriptionAdminClient.isInitialized()) {
      logger.info("Closing SubscriptionAdminClient")
      subscriptionAdminClient.value.close()
    }
    logger.info("GooglePubSubClient closed")
  }
}
