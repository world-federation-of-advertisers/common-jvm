      ...
      supervisorScope {
        ...
        while (true) {
          ...
          try {
            ...
            // Await result here so that any exceptions bubble up before attempting to commit.
            result = committableFuture.await()
            if (state != TransactionManager.TransactionState.STARTED) {
              // Something else changed the transaction state, e.g. by invalidating the transaction.
              // TODO(@SanjayVas): Figure out how this happens.
              throw SpannerExceptionFactory.newSpannerException(
                ErrorCode.ABORTED,
                "Transaction state changed to $state outside of AsyncTransactionManager",
              )
            }
