...
private class TransactionManagerImpl(private val delegate: AsyncTransactionManager) :
  ...
  private var lastTransactionStep: AsyncTransactionManager.AsyncTransactionStep<*, Any>? = null
  ...
  suspend fun commit(): Timestamp {
    ...
    check(lastTransactionStep != null) { "No mutations to commit" }
  ...
  private fun <T> runInTransaction(operation: (TransactionContext) -> ApiFuture<T>): ApiFuture<T> {
    ...
    return if (transactionStep == null) {
        ...
        transactionStep.then(
          { txn: TransactionContext, _ -> operation(txn) },
          MoreExecutors.directExecutor(),
        )
      }
      .also {
        @Suppress("UNCHECKED_CAST") // Only return type is relevant.
        lastTransactionStep = it as AsyncTransactionManager.AsyncTransactionStep<*, Any>
      }
  }
      ...
      while (true) {
        ...
        val result: R? =
          try {
            ...
            // Suppress exception here so it gets handled in commit.
            null
          }
        try {
          manager.commit()
