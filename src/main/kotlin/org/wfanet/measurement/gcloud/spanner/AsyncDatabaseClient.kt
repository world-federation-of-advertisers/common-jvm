...
private fun AsyncResultSet.asFlow(): Flow<Struct> {
  return callbackFlow {
      ...
      try {
        withContext(NonCancellable) {completionFuture.await()}
      } finally {
        close()
      }
      awaitClose()
...
private suspend fun <T> AsyncRunner.run(
  ...
  val txnFuture: ApiFuture<T> =
    runAsync(
      { txn ->
        // Cancellation here can break the transaction retry logic, so we use NonCancellable.
        apiFuture(NonCancellable) { doWork(txn) }
      },
      executor
    )
  txnFuture.await()
