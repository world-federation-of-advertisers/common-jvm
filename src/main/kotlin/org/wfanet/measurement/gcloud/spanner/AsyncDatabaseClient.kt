...
private class TransactionRunnerImpl(private val manager: AsyncTransactionManager) :
  ...
  companion object {
    private suspend fun <R> AsyncTransactionManager.runTransaction(doWork: TransactionWork<R>): R {
      ...
      while (true) {
        try {
          val result = coroutineScope { doWork(TransactionContextImpl(transactionFuture.await())) }

          transactionFuture
            .then(
              // No-op. This is relying on the fact that `buffer` is implemented as non-blocking,
              // so there's no reason to actually call `then` other than to get a handle to commit.
              { _, _ -> ApiFutures.immediateFuture(null) },
