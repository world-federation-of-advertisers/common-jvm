...
class S3StorageClient(internal val s3: S3Client, private val bucketName: String) : StorageClient {
  ...
  override suspend fun createBlob(blobKey: String, content: Flow<ByteString>): StorageClient.Blob {
    ...
    try {
      uploadParts(blobKey, content, uploadId)
      completeUpload(blobKey, uploadId)
    } catch (e: Exception) {
  ...
  private suspend fun uploadParts(blobKey: String, content: Flow<ByteString>, uploadId: String) {
    val digest = MessageDigest.getInstance("MD5")
    val builder = UploadPartRequest.builder().uploadId(uploadId).key(blobKey).bucket(bucketName)
    content.asBufferedFlow(BYTE_BUFFER_SIZE).withIndex().collect { (i, bytes) ->
      digest.update(bytes.asReadOnlyByteBuffer())
      val md5 = HexString(digest.digest().toByteString())

      val uploadPartRequest =
        builder.partNumber(i).contentLength(bytes.size().toLong()).contentMD5(md5.value).build()

      s3.uploadPart(uploadPartRequest, RequestBody.fromByteBuffer(bytes.asReadOnlyByteBuffer()))
    }
  }

  private fun completeUpload(blobKey: String, uploadId: String) {
  ...
  inner class Blob(private val blobKey: String, head: HeadObjectResponse) : StorageClient.Blob {
