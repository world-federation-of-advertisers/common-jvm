...
class SigningKeyStore(private val store: Store<Context>) {
  ...
  suspend fun write(privateKey: PrivateKey, certificate: X509Certificate): SigningKey {
    val pemBytes =
    ...
    return SigningKey(blob.blobKey, privateKey, certificate)
  }

  suspend fun read(keyId: String): SigningKey? {
    val blob = store.get(keyId) ?: return null
    return blob.read().flatten().newInput().reader().useLines { lines ->
      withContext(Dispatchers.IO) {
        val reader = PemReader(lines)
        ...
        SigningKey(blob.blobKey, privateKey, certificate)
      }
    }
  }
}
