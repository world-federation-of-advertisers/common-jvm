...
suspend fun StorageClient.createSignedBlob(
  ...
  val signer = keyHandle.newSigner()
  val outFlow = content.onEach { bytes -> signer.apply { update(bytes.asReadOnlyByteBuffer()) } }
  val blob = createBlob(blobKey, outFlow)
  val signature = ByteString.copyFrom(signer.sign())
