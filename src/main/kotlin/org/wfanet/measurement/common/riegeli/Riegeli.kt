...
object Riegeli {
  ...
  /**
   * builds a HighwayHash object with the key 'Riegeli/', 'records\n', 'Riegeli/', 'records\n'
   *
   ...
   *
   * @param incomingInputStream An input stream which contains data which should be decompressed
   *   using Riegeli. This input stream will be closed when the sequence has been exhausted or upon
   *   any error.
      ...
      incomingInputStream.use { incomingInputStream ->
        ...
        while (true) {
          ...
          if (chunk.chunkType == 0x72.toByte()) {
            ...
            // 0x73 - File Signature - Present at the beginning of the file, encodes no records, is
            // ignored
            // 0x6d - File Metadata - provides information describing the records, not necessary to
            // read, is ignored
            // 0x70 - Padding - encodes no records and only occupies file space, is ignored
            // 0x74 - Transposed ChuFnk with Records - no documentation provided for the format of
            // this chunk, unimplementable
   ...
   * @param dataSize ByteString containing the size of the chunk data, excluding intervening block
   *   headers. The ByteString should be storing a 64-bit, little-endian unsigned integer.
   ...
   *
   * @param numRecords ByteString representing the number of records inside the chunk. The
   *   ByteString should be storing a 56-bit, little-endian unsigned integer.
   * @param decodedDataSize ByteString representing the size of the chunk's data after it has been
   *   decoded. The ByteString should be storing a 64-bit, little-endian unsigned integer.
   * @param data ByteBuffer with the chunk's compressed data. The buffer's size is represented by
   *   dataSize.
   * @throws IllegalArgumentException if the chunk's header does not hash properly, meaning that the
   *   chunk header is corrupted.
   * @throws IllegalArgumentException if the hash of the chunk's data does not match dataHash within
   *   the chunk header, meaning the chunk's data is corrupted.
   */
  private class Chunk
  private constructor(
    ...
    init {
      require(
        isValidHeader(headerHash, dataSize, dataHash, chunkType, numRecords, decodedDataSize)
      ) {
        "Riegeli: Chunk header is invalid"
      }

      require(isValidData(dataSize, dataHash, data)) { "Riegeli: Chunk data is invalid" }
    }
    ...
    companion object {
       ...
       *
       * @param inputStream Counting input stream from which the chunk should be read. The input
       *   stream's count must have started at the beginning of a file.
      ...
      /**
       * Helper function for the chunk builder function. Compares the headerHash element of the
       * chunk's header to a highway hash of the chunk's other header elements.
       *
       * @param headerHash 64-bit ByteString containing a HighwayHash of the other chunk header
       *   elements (dataSize, dataHash, chunkType, numRecords, and decodedDataSize).
       * @param dataSize ByteString representing the size of the chunk data, excluding intervening
       *   block headers. The ByteString should be storing a 64-bit, little-endian unsigned integer.
       * @param dataHash ByteString containing a HighwayHash of the data within the chunk.
       * @param chunkType Byte containing a single byte which represents the chunk type.
       * @param numRecords ByteString representing the number of records inside the chunk. The
       *   ByteString is storing a 56-bit, little-endian unsigned integer.
       * @param decodedDataSize ByteString representing the size of the chunk's data after it has
       *   been decoded. The ByteString should be storing a 64-bit, little-endian unsigned integer.
      ...
      /**
       * Helper function for the chunk builder function. Compares the dataHash element of the
       * chunk's header to a highway hash of the chunk's data.
       *
       * @param dataSize ByteString representing the size of the chunk data, excluding intervening
       *   block headers. The ByteString should be storing a 64-bit, little-endian unsigned integer.
       * @param data ByteBuffer with the chunk's compressed data. The buffer's size is represented
       *   by dataSize.
       * @return True if the chunk's header is valid, false if it is not.
       */
     ...
     *
     * Compressed buffers, if compressionType is not 0, are prefixed with a varint containing their
     * decompressed size.
     *
     * @param dataBuffer The ByteBuffer from which the compressed buffer should be read.
     * @param compressedSize The size of the buffer, the number of bytes that should be read from
     *   the input stream.
     * @param compressionType A Byte representing the type of compression used in the compressed
     *   buffer.
     *     - 0x00: none
     *     - 0x62: Brotli
     *     - 0x7a: Zstd
    ...
    private fun readCompressedBuffer(
      ...
      // If compression type is 0, there is not a varint at the beginning of the buffer so do not
      // read it.
      val decompressedSize =
        if (compressionType == 0x00.toByte()) -1 else dataBuffer.getVarInt64().toInt()
      ...
      val compressedInputStream: CompressorInputStream =
     ...
     * - compressionType: A single byte representing which type of compression was used within the
     *   chunk.
     * - compressedSizesSize: A varint representing the size of compressedSizes
     * - compressedSizes: A compressed buffer of size compressedSizesSize. Contains numRecords
     *   varints, the size of each record.
     * - compressedValues: A compressed buffer with the record values. After decompression, contains
     *   decodedDataSize bytes.
     *
     * @return A list where each element is a ByteString representing a decompressed record. If
     *   there are no records within the chunk, an empty list is returned.
     */
    fun getRecords(): List<ByteString> {
      ...
      val numRecords =
        this.numRecords.withTrailingPadding(8).toLong(ByteOrder.LITTLE_ENDIAN).toInt()

      val sizesByteBuffer =
        readCompressedBuffer(dataBuffer, compressedSizesSizeInt, compressionType)
      sizesByteBuffer.rewind()
      val sizes = LinkedList<Int>()

      repeat(numRecords) { sizes.add(sizesByteBuffer.getVarInt64().toInt()) }

      val chunkBeginningSize = dataBuffer.position() - startingPoint
      val encodedDataSize =
        (this.dataSize.toLong(ByteOrder.LITTLE_ENDIAN) - chunkBeginningSize).toInt()
   ...
   * @param previousChunk ByteString representing the distance from the beginning of the chunk
   *   interrupted by this block header to the beginning of the block. The ByteString should be
   *   storing a 64-bit, little-endian unsigned integer.
   * @param nextChunk ByteString representing the distance from the beginning of the block to the
   *   end of the chunk interrupted by this block header. The ByteString should be storing a 64-bit,
   *   little-endian unsigned integer.
   * @throws IllegalArgumentException if the block header does not hash properly, meaning that the
  ...
  private constructor(
    ...
    init {
      require(isValid(headerHash, previousChunk, nextChunk)) { "Riegeli: Block header is invalid" }
    }
    ...
    companion object {
      ...
      /**
       * Helper function for the block header builder function. Compares the headerHash element of
       * the block's header to a highway hash of the block's other header elements.
       *
       * @param headerHash ByteString containing a HighwayHash of the other block header elements
       *   (previousChunk and nextChunk).
       * @param previousChunk ByteString representing the distance from the beginning of the chunk
       *   interrupted by this block header to the beginning of the block. The ByteString should be
       *   storing a 64-bit, little-endian unsigned integer.
       * @param nextChunk ByteString representing the distance from the beginning of the block to
       *   the end of the chunk interrupted by this block header. The ByteString should be storing a
