...
class MesosRecordIoStorageClientTest {
  ...
  @Test
  fun `test reading blob when record size and record delimiter are splitted across two chunks`() = runBlocking {
  ...
  private fun makeClientWithChunks(vararg chunks: ByteString) =
    MesosRecordIoStorageClient(object : StorageClient {
      ...
      override suspend fun listBlobs(prefix: String?) = throw UnsupportedOperationException("not used")
    })
