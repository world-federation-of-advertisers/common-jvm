...
def _run_protoc(ctx, proto_dep, output_dir):
    ...
    ctx.actions.run(
        ...
        progress_message = "Generating Kotlin protos for " + str(proto_dep.label),
    )
...
def _build_srcjar(ctx, proto_dep, input_dir, source_jar):
    ...
    ctx.actions.run_shell(
        ...
        progress_message = "Generating Kotlin proto srcjar for %s" % proto_dep.label,
    )
...
def _kt_jvm_proto_library_helper_impl(ctx):
    ...
    _run_protoc(ctx, proto_dep, gen_src_dir)
...
_kt_jvm_proto_library_helper = rule(
    attrs = dict(
        deps = attr.label_list(
            providers = [JavaInfo],  # This also shouldn't be needed
        ),
...
def kt_jvm_proto_library(
    ...
    if flavor and flavor != "normal":
        fail("Only 'normal' flavor is currently supported", "flavor")
