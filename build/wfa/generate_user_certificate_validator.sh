...
main() {
  ...
  # Check that user certificate is signed by root CA. Note that because we include a SAN in the cert, `-verify_hostname`
  # will return an error. This is tolerable.
  if ! openssl verify -x509_strict -verbose -CAfile "${root_certificate_pem_file}" "${user_certificate_pem_file}"; then
